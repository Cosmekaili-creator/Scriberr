...
func (u *UnifiedTranscriptionService) processSingleTrackJob(ctx context.Context, job *models.TranscriptionJob) error {
	...
	if job.Parameters.Diarize && diarizationModelID != "" {
		...
		if !u.transcriptionIncludesDiarization(transcriptionModelID, job.Parameters) {
			...
			if err != nil {
				// Soft-fail: transcription already succeeded, don't discard it
				logger.Warn("Diarization adapter unavailable, saving transcript without speaker labels",
				...
				if err != nil {
					// Soft-fail: transcription already succeeded, don't discard it
					logger.Warn("Diarization failed, saving transcript without speaker labels",
						"job_id", job.ID, "error", err)
				}
